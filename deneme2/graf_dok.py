# -*- coding: utf-8 -*-
"""graf_dok -- GRAF TUTARLILIGI: TERS ILISKILER, SIMETRI, TIP, GEREKTIRME.

    python graf_dok.py [--model model_b6]

`veri_dok` BOLMELERI denetler, `havuz_dok` EGITIM SATIRLARINI.
Bu betik GRAFIN KENDISINI denetler: "X anne Y" dediysek "Y cocuk X"
oluyor mu, ogretmen/ogrenci tam mi, simetrik iliskiler gercekten
simetrik mi, tipler tutuyor mu.

Kullanici, 16 Eylul: "Ayse Yilmaz anne Fatma Yilmaz dediysek Fatma
Yilmaz cocuk Ayse Yilmaz olmali. Bunlar tam mi? Bu tarz veri
tutarliligi var mi ogretmen ve ogrenci icinde?"
"""
import sys
import collections
...
for _p in (os.path.join(_D, "model_a"), os.path.join(_D, "model_b"),
           os.path.join(_D, "model_c"), _D):
    if _p not in sys.path:
        sys.path.insert(0, _p)
import model_a as M          # noqa: E402

_ap = argparse.ArgumentParser()
_ap.add_argument("--model", default="model_b6")
_a = _ap.parse_args()
AYAR = importlib.import_module(_a.model).AYAR
