# -*- coding: utf-8 -*-
"""tr_corpus_19 — CUMLELERDEN BELGE, BELGELERDEN PAKET. model_09'un havuzu.
...
model_16'nin korpus_16.py'sinden tasindi, icerik birebir (izler data_tr_19.REF_* ile sinanir).
Hat ve bu dosyanin yeri: data_tr_19.py'nin basligi.
"""
...
import tr_chars_19 as J
import tr_text_19 as MT
import tr_graph_19 as V
...
def sayfalar(v, G, kopya: int = 1, tohum: int = 0, tetik: int = 0,
    ...
    Sayfa IKI kaynaktan:
      ...
      ANILAN    (a, r) -> e        e'yi NESNE yapan olgular, TERS IFADEYLE
                                   yeniden yazilir (tr_text_19.TERS).
...
def reddetme_belgeleri(v, G, n, tohum=0, grup=6, bolme=None, yaz=print):
    ...
    Ayrinti `tr_graph_19.IMKANSIZ` basindaki notta.
...
def yasak_ciftler(v):
    ...
    yalniz kendi sahibinin olgularini soyluyordu. model_14'da sayfa
...
def _havuz_kapisi(X, dilim, S, t_len, yaz=print):
    ...
    yaz(f"  KORPUS IZI {S.korpus_izi}   -- tensorun kendisi. `tr_text_19` ya "
        f"da `tr_corpus_19` degisirse bu deger degisir, `ayar` DEGISMEZ.")
