# -*- coding: utf-8 -*-
"""model_02 — KLASIK decoder-only transformer. Bu projenin HICBIR ozelligi YOK.
...
`test_02.py` her kosuda sinar: VERI alanlari model_b15 ile birebir
...
# EKLENMEZ -- kullanici karari, 16 Eylul: "model_02 diger hicbir model
...
assert hasattr(M, "egit"), (
    f"taban_02 MODUL degil PAKET olarak yuklendi: {getattr(M,'__file__',None)}")
...
class ModelSade(nn.Module):
    ...
    def __init__(self, ayar: M.Ayar, vocab: int):
        super().__init__()
        assert ayar.dongu == 1, (
            f"model_02 DONGUSUZ: dongu={ayar.dongu}. Dongu istiyorsan "
            f"model_b ailesini kullan.")
        assert ayar.dar_alfa == 0 and not ayar.dar_kapi, (
            "model_02'da Phi DARBOGAZI YOK -- dar_alfa=0, dar_kapi=False")
        assert ayar.kopru_kayip == 0, "model_02 YARDIMCI KAYIP KULLANMAZ"
        assert ayar.mask_poz is None and not ayar.mask_blok, (
            "model_02'da MASKE YOK")
        assert ayar.dusun_gecis >= 1, (
            f"dusun_gecis >= 1 olmali: {ayar.dusun_gecis}")
        assert ayar.dusun_tau > 0, f"dusun_tau > 0 olmali: {ayar.dusun_tau}"
        self.ayar = ayar
    ...
    def govde(self, x, ek=None):
        """SON katman + son norm -- head ONCESI gizli durum (B, T, d).

        `ek` (B, T, d): giris gommesine EKLENIR. Yumusak kopru dongusu
        ikinci gecisi bununla besler; None ise hicbir sey degismez.

        `asama1_02.gizli()` (DOGRUSAL SONDA) bunu cagirir. KUSUR ve
        ...
        h = self.emb(x)
        if ek is not None:
            assert ek.shape == h.shape, (ek.shape, h.shape)
            h = h + ek.to(h.dtype)
    ...
    def son_gizli(self, x):
        """BUTUN gecislerden SONRAKI gizli durum (B, T, d).

        !! DOGRUSAL SONDA VE HER TESHIS BUNU CAGIRMALI, `govde()`yi
        DEGIL. `dusun_gecis > 1` iken `govde(x)` yalnizca BIRINCI
        gecistir; modelin cevabi uretirken kullandigi durum SONUNCU
        gecisinkidir. 16 Eylul'de sonda `model_a`ya ozgu bir ileri
        gecisi elle kurdugu icin kirilmisti; ayni tuzagin bu koldaki
        hali, sondanin YANLIS GECISI olcmesi olurdu -- sessizce, ve
        20.000 adimlik kosudan sonra.
        """
        h = self.govde(x)
        for _ in range(self.ayar.dusun_gecis - 1):
            # CIKIS uzayi -> GIRIS uzayi. `head.weight is emb.weight`
            # oldugu icin bu round-trip SIFIR yeni parametre kullanir.
            p = torch.softmax(self.head(h) / self.ayar.dusun_tau, dim=-1)
            z = p @ self.emb.weight                     # (B,T,V)@(V,d)
            h = self.govde(x, ek=self.ayar.dusun_alfa * z)
        return h

    def forward(self, x):
        # KAYIP YALNIZ SON GECISE uygulanir; ara gecisler DENETLENMEZ.
        return self.head(self.son_gizli(x))

    @torch.no_grad()
    def dusun_dagilimi(self, x):
        """ILK gecisin dagilimi p1 (B, T, V) -- KOPRU oraya mi yaziliyor?

        Onkayitta yazili KRITIK OLCU: p1 yayvansa z onlarca varligin
        ortalamasi olur ve ikinci arama BULANIK anahtarla calisir.
        Egitimde CAGRILMAZ, yalniz teshiste."""
        assert self.ayar.dusun_gecis > 1, "dongu KAPALI -- ara gecis YOK"
        return torch.softmax(self.head(self.govde(x)) / self.ayar.dusun_tau,
                             dim=-1)
...
# `ayar_02.py`de tek tek yazili -- ve artik `model_b15`ten DEVRALINMIYOR,
# `Ayar()` varsayilaninin uzerine acikca yaziliyor.
from ayar_02 import AYAR, GOREV_ALAN                         # noqa: E402,F401
