# -*- coding: utf-8 -*-
"""asama1_02 — model_02'in KENDI ASAMA-1 TESHISI. TEK BASINA DURUR.
...
# `asama1_b` bunu doldurur; None -> model_02.ModelSade. `pencere_a` ve
...
def gizli(net1, v, lst, poz, bs=256):
    ...
    for i in range(0, len(X), bs):
        ...
        # !! `govde` DEGIL `son_gizli`: dusun_gecis>1 iken govde
        # yalnizca BIRINCI gecistir, cevap SONUNCUDAN uretilir.
        cik.append(net1.son_gizli(xb)[:, poz].cpu())
    ...
    for bol in a.bolme.split(","):
        ...
        if a.head and getattr(net, "cok_bas", False):
            ...
            with torch.no_grad():
                ...
                hh = net1.son_gizli(torch.from_numpy(X0).to(M.DEV))
