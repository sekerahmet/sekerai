# -*- coding: utf-8 -*-
"""taban_02 — model_02'in KENDI motoru. TEK BASINA DURUR.
...
Bu dosya `model_a.py`nin KOPYASIDIR ve artik model_02'a aittir.
...
KENDILIGINDEN gelmez; model_02 ile model_b15 O GUNDEN SONRA FARKLI
KODLA olculmus olur. Bu, kopyanin gercek riski ve gozden kacarsa
sayilari sessizce karsilastirilamaz hale getirir.

Bunun icin `test_02.py` her kosuda DAVRANIS ESDEGERLIGI siniyor:

    egitim havuzu       taban_02 vs model_a  -> BIT DUZEYINDE ayni
...
# import ediyordu. `taban_02` bunu YAPMIYOR: model_02'in verisi
# `veri_02.py`, ve o BU klasorde. Veri modulu `veri_kur` icinde
...
class Ayar:
    ...
    veri_ad: str = "veri_02"     # HANGI GRAF. "veri_okul2" = tam IKI KATI.
    ...
    # --- YUMUSAK KOPRU DONGUSU -- BU KOLUN DUGMESI ---------------------
    # Govde `dusun_gecis` kez kosar. Gecisler arasinda, ILK gecisin
    # gizli durumu jeton dagilimina cevrilip GERI GOMULUR:
    #
    #     p = softmax(E h / tau)          jetonlar uzerinde dagilim
    #     z = E^T p                       YUMUSAK JETON (gomme uzayinda)
    #     h = govde(x, ek = alfa * z)     ikinci gecis
    #
    # NEDEN. Bir tablo aramasi cevabi CIKIS uzayinda uretir; ikinci
    # arama anahtarini GIRIS uzayinda ister. CoT bu cevirmeni jetonu
    # YAZIP GERI OKUYARAK yapar. Bagli gomme (head.weight is emb.weight)
    # ayni cevirmeni jeton yazmadan veriyor -- SIFIR yeni parametre.
    #
    # GEREKCE OLCULDU (17 Eylul): model iki 1-hop aramasini da yapiyor
    # (1.0000 / 0.9997) ama birlestiremiyor (0.0550); kopru soru sonunda
    # DOGRUSAL OKUNAMIYOR (0.0482, taban 0.0361) ve h(e1,r1) ~ h(e2)
    # hizalanmasi YOK (+0.0053). Eksik olan devre degil, TESISAT.
    #
    # !! KAYIP YALNIZ SON GECISE uygulanir -- ara gecis DENETLENMEZ.
    # Boylece ilk gecisin ne uretecegi serbest kalir; tek kisit, aranin
    # gomme satirlarinin DISBUKEY BIRLESIMI olmak zorunda olmasi.
    # Bu kisit ara adimi "varlik seklinde" olmaya zorluyor.
    #
    # dusun_gecis = 1  ->  DEGISMEDI, model_00 ile BIT DUZEYINDE ayni.
    dusun_gecis: int = 1
    dusun_tau: float = 1.0     # sicaklik; dustukce kopru keskinlesir
    dusun_alfa: float = 1.0    # geri gomulen z'nin agirligi
...
ESKI_VARSAYILAN = {
    ...
    # Yumusak kopru dongusu 17 Eylul'de eklendi; ondan onceki butun
    # kosular govdeden TEK KEZ geciyordu -> 1 = kapali.
    "dusun_gecis": 1, "dusun_tau": 1.0, "dusun_alfa": 1.0,
...
# kosulabiliyordu. `taban_02` bir MOTOR; model_02'in ayari `ayar_02.py`de,
# kosuyu baslatan `model_02.py`. Burada calistirilacak bir sey YOK.
