...
txt olarak cikarir misin."

NE URETIR (hepsi duz metin, elle okunabilir):

    00_OZET.txt          sayilar + SAGLIK DENETIMI hukumleri
    01_tokenlar.txt      butun sozluk, id -> token, nerede geciyor
    02_varliklar.txt     2120 varlik, jeton ayrisimi, tipi
    03_iliskiler.txt     17 iliski + tip semasi + dagilim
    04_egitim_1hop.txt   EGITIM: atomik olgular
    05_egitim_2hop.txt   EGITIM: gorulen zincirler (ezber havuzu)
    06_sinav_comp.txt    SINAV: ucluyu gormedi
    07_sinav_ent.txt     SINAV: varlik hic zincir basi olmadi
    08_sinav_ent_yok.txt SINAV: ent + kisayol TIP OLARAK imkansiz
    09_sinav_ood.txt     SINAV: iki kenar da tutulmus (BIRINCIL)
    10_saglik.txt        denetimlerin AYRINTISI (ornekleriyle)
    11_taban_cizgiler.txt AKILSIZ stratejiler kac alir (SANS SEVIYESI)

Her sinav satirinda SORU, KOPRU, CEVAP ve KISAYOL yan yana durur; yani
dosyaya bakan biri modelin isini ELLE yapabilir.
"""
...
for _p in (os.path.join(_K, "model_a"), os.path.join(_K, "model_b"), _K):
