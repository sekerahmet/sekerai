# -*- coding: utf-8 -*-
"""analiz_00 — model_00'in KENDI VERI DOKUMU + SAGLIK DENETIMI. TEK BASINA DURUR.

Kullanici karari, 16 Eylul 2026: *"bunlarin hepsi model_00 folderi
altinda olmali. model_00 diger hicbir model ile ayni seyi kullanmamali.
Analiz icinde analiz_00 kullanalim mesela, digerleri icin de."*

`veri_dok.py`nin KOPYASI (uretici: scratchpad/kur_analiz00.py). Iki fark:
motor `taban_00`, ve `--model` secenegi YOK -- bu arac yalniz model_00'i
tanir. Dokum `model_00/veri/model_00/` altina gider (depoya girmez).

    python analiz_00.py [--tam] [--klasor <yol>]
"""
...
def main():
    ...
    # `--model` YOK: bu arac yalniz model_00'i tanir.
    ap.set_defaults(model="model_00")
