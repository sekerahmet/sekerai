# -*- coding: utf-8 -*-
"""model_10 — KLASIK decoder-only transformer. Bu projenin HICBIR ozelligi YOK.
...
`test_10.py` her kosuda sinar: VERI alanlari model_b15 ile birebir
...
assert hasattr(M, "egit"), (
    f"taban_10 MODUL degil PAKET olarak yuklendi: {getattr(M,'__file__',None)}")
...
class ModelSade(nn.Module):
    ...
    def __init__(self, ayar: M.Ayar, vocab: int):
        super().__init__()
        assert ayar.dongu == 1, (
            f"model_10 DONGUSUZ: dongu={ayar.dongu}. Dongu istiyorsan "
    ...
    def govde(self, x):
        ...
        `asama1_10.gizli()` (DOGRUSAL SONDA) bunu cagirir. KUSUR ve
...
# `ayar_10.py`de tek tek yazili -- ve artik `model_b15`ten DEVRALINMIYOR,
# `Ayar()` varsayilaninin uzerine acikca yaziliyor.
from ayar_10 import AYAR, GOREV_ALAN                         # noqa: E402,F401
