# -*- coding: utf-8 -*-
"""egitim_dok_10 — VERININ KENDISI, OKUNAN metin.
...
cagirdigi AYNI kodlayicilarla kuruluyor.
    ...
    python egitim_dok_10.py [--klasor <yol>] [--limit N]
"""
...
# duruyor -- ama ANAHTARLARI `veri_10.ILISKI` ile birebir tutmak ZORUNDA:
...
def kur_03_iliskiler(d, v, H):
    ...
    assert not eksik, (
        f"ILISKI_NOT ile veri_10.ILISKI TUTMUYOR: {sorted(eksik)} "
...
def _olcu_kapisi(bolumler, yaz=print):
    ...
    import re
    kaynak = io.open(os.path.join(_K, "pencere_10.py"),
                     encoding="utf-8").read()
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=LIMIT,
                    help=f"her TIPten en fazla N satir (varsayilan {LIMIT}, "
                         "0 = HEPSI)")
    a = ap.parse_args()
    kl = a.klasor or os.path.join(_K, "veri", "model_10")
    ...
    with io.open(yol, "w", encoding="utf-8", newline="") as f:
        f.write("model_10 — VERI DOKUMU" + NL)
        f.write("=" * 72 + NL + NL)
        ...
        f.write("sirali, ilk N satir hep ayni birkac varligi gosterirdi."
                + NL)
        f.write("Tamamini dokmek icin:  python egitim_dok_10.py --limit 0"
                + NL + NL)
