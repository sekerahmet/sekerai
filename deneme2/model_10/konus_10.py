# -*- coding: utf-8 -*-
"""konus_10 — model_09 ile DUZ KONUSMA. Sablon YOK, denetim YOK.
...
verilir (onkayit `belge/onkayit/model_10.md`).

    python konus_10.py                      son 5'in AGIRLIK ORTALAMASI
    python konus_10.py --adim 8000          belli bir adim
    python konus_10.py --genislik 5         son 5'in agirlik ortalamasi
    python konus_10.py --klasor <yol>       baska bir kosu
...
# kismi CALISMAZ.
#
# BAYRAK NEDEN VAR: bu kolda AYNI kusur IKI KEZ oldu -- `kos_10.py`
# icinde `MODEL="model_08"`, `ayar_10.py` icinde `from taban_08 import
...
# terminalden, IDE'den, duz `python konus_10.py` ile -- Windows yerel
...
class Sozluk:
    ...
    def __init__(self, v, VM):
        ...
        # (unlu uyumu; `taban_10._nin` / `_dir` ile AYNI tablo).
        self.nin, self.dir = {}, {}
...
def dene(D, S, n=300):
    """JETONLAYICI SINAMASI -- `python konus_10.py --dene`.
    ...
    Kilit testine (`test_10.py`) KONMADI: o test kosudan once calisan
...
# (`veri/model_10/10_sinav.txt`) satir kopyalayip kisaltarak test etti:
#
...
def main():
    ...
    # COZUCU: `analiz_10.Dok` jeton dizisini okunabilir Turkce'ye cevirir
    ...
    assert (D.v.vocab, D.v.n_ent, D.v.ek0) == (v.vocab, v.n_ent, v.ek0), (
        "depodaki model_10.AYAR ile KOSUNUN ayari ayristi -- sozluk tutmuyor")
    D.v = v                      # kosunun kendi verisi okunsun
    S = Sozluk(v, D.VM)

    print(f"model_10 t{ayar.tohum}   anlik goruntu "
    ...
    # `veri/model_10/` altinda; bu yalniz elin altinda dursun diye.
    TR = D.VM.TR
