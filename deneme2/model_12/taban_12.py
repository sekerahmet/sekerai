# -*- coding: utf-8 -*-
"""taban_12 — BU KOLUN KENDI motoru. TEK BASINA DURUR.
...
Bunun icin `test_12.py` her kosuda DAVRANIS ESDEGERLIGI siniyor:
...
# `veri_12.py`, ve o BU klasorde. Veri modulu `veri_kur` icinde
# `ayar.veri_ad`dan import ediliyor (asagida), burada degil.

import jeton_12 as J
import dil_12 as DIL
import korpus_12 as KOR
import olcme_12 as OLC

DEV = "cuda" if torch.cuda.is_available() else "cpu"

# Ozel token'lar. ENT_OFF/VOCAB veriden TURER (Veri.__post_init__),
# burada sabit YAZILMAZ.
# OZEL BLOK 8 -> 3 (kullanici, 17 Eylul: "silinsin tabii ki"):
# sekiz ozel jetonun BESI havuzda hic gecmiyordu. `ek_kip` ""/"tr"
# kodlama yollari da silindi; yalniz "tr2" kaldi.
...
class Ayar:
    ...
    veri_ad: str = "veri_12"     # HANGI GRAF. "veri_okul2" = tam IKI KATI.
    ...
    # --- KORPUSUN BILESIMI (model_12). Hepsi AYARDAN gelir ki onkayit
    ...
    # --- HIBRIT KARISIM (model_12) ----------------------------------
    # `karisim` her blogun DIZI KARISTIRICISINI sec: "A" tam dikkat,
    # "G" Gated DeltaNet-2. Uzunlugu `l` kadar olmali. BOS = eski
    # davranis, yani her blokta tam dikkat (model_11).
    #   AYARDA duruyor ki `ayar_t<N>.json`a girsin: "bu kosu hangi
    #   mimariydi" sorusu SONRADAN cevaplanabilsin. Kaynak koda
    #   gomulseydi yalniz commit hash'inden okunurdu.
    karisim: str = ""          # ornek: "GGGAGGGA" -> 3:1, dikkat 4 ve 8'de
    gdn_v_kat: int = 2         # GDN-2 deger kafasi carpani (Hv = nh * bu).
    #   Qwen3-Next ile ayni (2x). Makale 2605.22791 C.1 grupla(nmis)
    #   deger kafalarini acikca destekliyor: "the key-side tensors q, k,
    #   the log-decay tensor g, and b are repeated across the value-head
    #   group". 1 yazilirsa genisleme YOK.
    ...
    optim: str = "adamw"       # "adamw" | "muon"
    #   MUON 2026 uretim tarifi: GLM-4.5/GLM-5, Kimi K2, DeepSeek-V4
    #   gecti, Kimi K3 Per-Head Muon kullaniyor. PyTorch'ta NATIVE
    #   (`torch.optim.Muon`), ucuncu parti YOK.
    #   !! Muon YALNIZ 2B parametre kabul ediyor -- OLCULDU, 1B'de
    #   `ValueError: Muon only supports 2D parameters`. Zaten standart
    #   tarif de boyle: gizli matrisler Muon'a, gomme/unembedding ve
    #   skalerler AdamW'ye. `_optim_kur` bu ayrimi yapar.
    ...
    #   phi <- (1-alfa)*phi + alfa*theta ; theta <- phi
    #   ort_bas=0 -> KAPALI (varsayilan).
    # --- DARBOGAZ: dar_* alanlari OLU, hicbir kol kullanmiyor.
    ...
    #   "tam"  BUTUN alt cizgilerden boler, TEK PAYLASILAN sozluk,
    #          3 yuvaya sagdan doldurur. TEK KULLANILAN deger.
    #          ("" ve "ilk" yollari SILINDI -- model_b5 kusurlu
    #          ilan etmisti, ayrinti belge/onkayit/model_b5.md.)
    kopru_kayip: float = 0.0   # YARDIMCI KAYIP: kopruyu TAHMIN ETTIR.
    #   0.0 = KAPALI. >0 ise r1/r2 pozisyonlarinda koprunun ilk iki
    #   token'i yardimci kayip olarak uretilir. BIR TESHIS, mimari
    #   oneri DEGIL -- kopru etiketi graftan gelir, gercek metinde
    #   gelmezdi. Hicbir kol kullanmiyor.
...
def veri_kur(ayar: Ayar, yaz=print) -> Veri:
    ...
    #     pencere_12  : olcme_listeleri -> egitim_havuzu   (TERS SIRA)
...
# ======================= KODLAMA ====================================
# model_08in jeton-yuvasi kodlayicilari SILINDI (18 Eylul, 453 satir):
# korpus artik KARAKTER duzeyinde -- metin_<N> uretir, jeton_<N>
# cevirir, korpus_<N> pencereye koyar.
...
# ======================= MODEL ===========================================
class IkiliOptim:
    """Muon (2B gizli matrisler) + AdamW (gomme, 1B, evrisim) TEK NESNE.

    Egitim dongusu `opt`tan sunlari istiyor ve hepsi burada:
    `state_dict` / `load_state_dict` / `param_groups` / `zero_grad` /
    `step`. `param_groups` ikisinin listesini BIRLESTIRIR -- LR programi
    tek dongude ikisini birden gunceller (sozlukler AYNI nesneler).

    !! GradScaler YOLU DESTEKLENMIYOR (`scaler.step(opt)`). Gerek de yok:
    Muon kullanan kol bf16 kosuyor ve bf16'da scaler ZATEN kapali."""
...
def _optim_muon(model, ayar, yaz=print):
    ...
    tarifin kendisi: Newton-Schulz ortogonallestirmesi bir MATRIS islemi.

    !! `adjust_lr_fn="match_rms_adamw"`: Muon'un ham LR olcegi AdamW'nin
    kinden BASKA (tipik Muon ~0.02, AdamW ~1e-3). Bu secenek guncelleme
    RMS'ini AdamW'ninkine esitliyor, boylece `ayar.lr` TASINABILIYOR ve
    kol "yanlis LR yuzunden dustu" belirsizligine dusmuyor. Desteklenen
    degerler OLCULDU: 'original', 'match_rms_adamw', None."""
    gom = model.emb.weight
    muon_p, ad_dec, ad_nodec = [], [], []
    ...
    yaz(f"  OPTIM muon: {len(muon_p)} matris {n(muon_p):,} param  |  "
        ...
        f"{n(ad_dec)+n(ad_nodec):,} param")
    mu = torch.optim.Muon(muon_p, lr=ayar.lr, weight_decay=ayar.wd,
                          momentum=0.95, nesterov=True,
                          adjust_lr_fn="match_rms_adamw")
    aw = torch.optim.AdamW([{"params": ad_dec, "weight_decay": ayar.wd},
                            {"params": ad_nodec, "weight_decay": 0.0}],
                           lr=ayar.lr, betas=tuple(ayar.betas))
    return IkiliOptim(mu, aw)
...
#
# Yerine `olcme_12.Sinav` + `olcme_12.olc`: model soruyu alir, gerisini
...
def surdurme_oku(yol, ayar: Ayar, model, opt, scaler, rs, iz, yaz=print,
    ...
    # ikisi de EGITIM METNINI kollamiyor. `metin_12.cumle`de tek bir ek
...
def erken_teshis(r: dict, ayar: Ayar, yaz=print, uyarildi: set | None = None):
    ...
    if k is not None and k == k and abs(k) > 1e-9:      # k == k  ->  NaN degil
        raise SystemExit(
            ...
            + os.linesep
            + "   isaret edecegi varlik GRAFTA YOK. `olcme_12.sorular` o"
            + os.linesep
...
def egit(ayar: Ayar, alt=None, yaz=print, ustune=False, commit=None,
    ...
    if str(getattr(ayar, "optim", "adamw")).lower() == "muon":
        opt = _optim_muon(model, ayar, yaz)
    else:
...
# kosulabiliyordu. `taban_09` bir MOTOR; model_09'in ayari `ayar_12.py`de,
# kosuyu baslatan `model_12.py`. Burada calistirilacak bir sey YOK.
