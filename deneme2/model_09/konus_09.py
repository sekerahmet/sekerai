# -*- coding: utf-8 -*-
"""konus_09 — model_09 ile DUZ KONUSMA. Sablon YOK, denetim YOK.
...
BU ARAC NEDEN VAR -- oncesinde SABLONLU bir sorucu vardi ve ise yaramadi:
    ...
    konus_09   yazdigin cumleyi JETONLAYIP modele verir, gerisini MODEL
...
!! BU BIR OLCU DEGIL. Elle secilmis sorulardir; hukum `pencere_09` ile
verilir (onkayit `belge/onkayit/model_09.md`).

    python konus_09.py                      son 5'in AGIRLIK ORTALAMASI
    python konus_09.py --adim 8000          belli bir adim
    python konus_09.py --genislik 5         son 5'in agirlik ortalamasi
    python konus_09.py --klasor <yol>       baska bir kosu

model_09'DE UC TURLU KONUSABILIRSIN. `model_06`da yalniz ilk ikisi
...
# ======================= !! TASINMADI ===================================
# Bu dosya HALA model_08'IN JETON SURUMU. model_09 karakter duzeyine
# gecti (18 Eylul): `kodla_1hop` / `kodla_2hop` / `v.yuva_ara` /
# `jeton_ad` semasinin hicbiri artik YOK, yani asagidaki kodun buyuk
# kismi CALISMAZ.
#
# BAYRAK NEDEN VAR: bu kolda AYNI kusur IKI KEZ oldu -- `kos_09.py`
# icinde `MODEL="model_08"`, `ayar_09.py` icinde `from taban_08 import
# Ayar`. Dosya adi degismis, ICERIK degismemisti. Sessiz duran bir
# kopya, calistirilana kadar dogru gorunur.
#
# `TASINMADI` True oldugu surece `kur()` ve `main()` DURUR ve sebebini
# soyler. `test_09` §8 bu bayragi kilitliyor: karakter surumu yazilinca
# bayrak dusecek ve §8 yeniden DAVRANIS sinayacak.
TASINMADI = True
_TASINMADI_MESAJ = (
    "konus_09 HALA model_08'in JETON surumu -- model_09 KARAKTER "
    "duzeyinde calisiyor." + chr(10)
    + "   Karakter surumu yazilana kadar bu arac KOSMAZ; kostursaydi "
    "model_08'in" + chr(10)
    + "   jeton semasini arayip AttributeError ile duserdi ya da daha "
    "kotusu," + chr(10)
    + "   YANLIS bir seyi dogru gibi basardi." + chr(10)
    + "   Kosuya ENGEL DEGIL: bu bir okuma araci, egitim yolunda yok."
)


def _tasinmadi_kapisi():
    if TASINMADI:
...
# terminalden, IDE'den, duz `python konus_09.py` ile -- Windows yerel
...
# ayni ders test_09 §9'da alt surec cagrisinda zaten alinmisti
...
import taban_09 as M                                          # noqa: E402
import pencere_09 as P                                        # noqa: E402
import analiz_09 as AZ                                        # noqa: E402
from model_09 import ModelSade                                # noqa: E402

# Ham kosu verisi Drive'da, yerelde bagli (CLAUDE.md "Nerede ne okunur").
KLASOR = os.path.join("G:" + os.sep, "Drive'ım", "model_09", "t0")

# Turkce harfleri ASCII'ye indirir. Kullanici "Ayse" de yazabilsin
# "Ayşe" de -- jeton adlari ASCII (par_ad), ekranda gosterilen bicim
# Turkce (veri_09.TR). Ikisi de ayni anahtara duser.
#
...
class Sozluk:
    ...
    def __init__(self, v, VM):
        ...
        assert v.paylasilan, "konus_09 PAYLASILAN sozluk bekler"
        self.kel = {}
        ...
        # (unlu uyumu; `taban_09._nin` / `_dir` ile AYNI tablo).
        self.nin, self.dir = {}, {}
...
def kur(klasor, genislik=1, adim=None):
    ...
    !! AYRIM: KURAL bu kolun, OLCUMLER model_09'un DEGIL. Asagidaki
    butun sayilar `model_08`de olculdu; model_09 HENUZ KOSMADI.
    (Bu dosya model_08'den kopyalandi, 18 Eylul.)

    Kural: *arac, HUKMUN DAYANDIGI modeli gostermeli; baska bir modeli
    ...
    O yuzden varsayilan 5 yapilmisti.

    VARSAYILAN 1 OLMASININ SEBEBI: `model_09` de -- `model_08` gibi --
    GERI BESLEMELI bir kol; agirlik ZATEN egitim icinde ortalaniyor ve
    onkayit §3 birincil okumayi EGRI yaziyor. Dayanak ONCEDEN YAZILMIS
    OKUMA KURALI, bir olcum degil.

    !! NEDEN OLCUME DAYANDIRILMIYOR -- model_08'de DENENDI VE CURUDU.
    Once "bu kolda pencere ZARAR veriyor" diye olcume dayali bir gerekce
    yazildi; 20.000'de dogruydu, 40.000'de DORT olcude de ISARET DONDU:

        model_08     EGRI   PENCERE(5)   pencere - egri
    ...
    Sebep anlasilir: pencere, EGRI DIKKEN eski (kotu) anlik goruntuleri
    karistirdigi icin geri ceker; egri DUZLESINCE ayni islem yalniz
    gurultuyu siler ve yardim eder. Yani "hangisi daha iyi" ADIMA BAGLI
    ve bir varsayilan ona dayandirilamaz.

    model_08'de AYAKTA KALAN: iki okuma arasindaki fark KUCUK (|0.02|
    civari, en cok 0.056); `model_07`de ayni fark +0.18'e kadar cikiyor
    ve BUYUYOR. **model_09 icin bu bilinmiyor** -- jetonlama ve veri
    bicimi degisti, olcum yenilenmeli.
...
def devam(v, net, jet, ornekle=False, isi=1.0, tohum=None):
    ...
    # gosterir; hukum yine `pencere_09`in (o ARGMAX olcer, olcum
...
def dene(D, S, n=300):
    """JETONLAYICI SINAMASI -- `python konus_09.py --dene`.
    ...
    Kilit testine (`test_09.py`) KONMADI: o test kosudan once calisan
...
# (`veri/model_09/10_sinav.txt`) satir kopyalayip kisaltarak test etti:
#
...
def main():
    ...
    # COZUCU: `analiz_09.Dok` jeton dizisini okunabilir Turkce'ye cevirir
    ...
    assert (D.v.vocab, D.v.n_ent, D.v.ek0) == (v.vocab, v.n_ent, v.ek0), (
        "depodaki model_09.AYAR ile KOSUNUN ayari ayristi -- sozluk tutmuyor")
    D.v = v                      # kosunun kendi verisi okunsun
    S = Sozluk(v, D.VM)

    print(f"model_09 t{ayar.tohum}   anlik goruntu "
    ...
    # `veri/model_09/` altinda; bu yalniz elin altinda dursun diye.
    TR = D.VM.TR
