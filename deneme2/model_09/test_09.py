...
Dort sey tutulur:
  ...
  1. BAGIMSIZLIK  model_09/*.py icinde `model_a` / `model_b` /
  ...
  3. VERI         `veri_09` KENDI iddialarini tutuyor: sema, soy
...
# --- 0) BAGIMSIZLIK ------------------------------------------------------
print("=== 0) BAGIMSIZLIK: model_09/ disariya BAGLI MI ===")
# Metinde arama YAPMIYORUZ -- ilk denemem oyleydi ve `ayar_09.py`nin
...
import model_09 as S                                         # noqa: E402
...
ok(not issubclass(S.ModelSade, M.Model),
   "ModelSade, taban_09.Model'den MIRAS ALMIYOR")
...
# KUSUR (16 Eylul hakemligi): `asama1_09.gizli()` ileri gecisi ELLE
...
import asama1_09 as A1                                      # noqa: E402
import olcme_09 as OLC                                      # noqa: E402
import veri_09 as V00                                       # noqa: E402
_lst = v.comp[:8]
...
# Gercek kullanimda `asama1_09.main` modeli zaten `.to(M.DEV)` ediyor;
...
try:
    ...
    ok(_q.shape == (8, A.d), "asama1_09.gizli() KOSUYOR (DOGRUSAL SONDA yolu)",
    ...
    ok(False, "asama1_09.gizli() KOSUYOR (DOGRUSAL SONDA yolu)",
...
print("\n=== 3) VERI: veri_09 KENDI IDDIALARINI tutuyor mu ===")
import veri_09 as V00                                        # noqa: E402

ok(A.veri_ad == "veri_09", "BU KOL KENDI veri modulunu okur", A.veri_ad)
_G0 = V00.kur(0)
E05 = [x for t in V00.TIPLER for x in _G0["ad"][t]]
_z = V00.zincirler(_G0)
ok(V00.graf_izi(_G0) == V00.IZ, "graf izi veri_09.IZ ile TUTUYOR", V00.IZ)
...
_bek = (["'"] + list(V00.EK_NIN) + list(V00.EK_DIR)
        ...
        + ["<BOS>", "<AYIR>", ","])
ok(_ekler == _bek, "analiz_09.jeton_ad EK JETONLARINI adlandiriyor",
   f"{len(_ekler)} jeton")
...
for _g in ("kur", "zincirler", "TIPLER", "ILISKI", "SEMA", "GEREKTIRIR",
           ...
           "SEHIR_BOLGE", "yuzey"):
    ok(hasattr(V00, _g), f"veri_09.{_g} var (sozlesme)")
...
print("\n" + "=== 4) BILDIRILMIS AYRISMA: model_09 <-> model_08 ===")
import model_a as MA                                         # noqa: E402
import metin_09 as MT                                        # noqa: E402
import olcme_09 as OLC                                       # noqa: E402
from ayar_09 import GOREV_ALAN                               # noqa: E402
...
ok(_alan09 - _alanA == {"fim_kat", "soru_kat", "kisayol_kat",
                        "t_len", "kopya", "tetik"},
   "Ayar'a EKLENEN alanlar: fim/soru/kisayol (06-09) + t_len/kopya/tetik",
...
ok(S0.vocab == 63, "sozluk 63 jeton (61 karakter + PAD + EOS)", str(S0.vocab))
...
_mar = _gecis * 10          # kopya 5 x (bildirim + soru)
ok(425 <= _mar <= 460,
   "maruziyet model_08'inkine (442) ESIT -- butce turetimi tutuyor",
   f"{_mar:.0f} maruziyet / {_gecis:.1f} gecis / {_jeton:,} jeton")
...
# (`olcme_09.puanla`) ve burada SAHTE CIKTIYLA sinaniyor.
...
# emsali). model_09..model_07'de `ort_bas == 0` bekleniyordu: "standart
# tarif, projeye ozgu ne varsa KAPALI". model_09'de geri beslemeli
# ortalama BU KOLUN DUGMESI (onkayit belge/onkayit/model_09.md), o
...
# Asagidakilerin hepsi `model_09.py` basindaki tabloda YAZILI; yazili
...
for g in ("AYAR", "egit", "fark_bas", "ModelSade", "TABAN"):
    ok(hasattr(S, g), f"model_09.{g} var", "kos_09.py duser")
...
for ad in ("veri_09", "taban_09", "ayar_09", "pencere_09", "tani_09",
           "asama1_09", "kos_09", "konus_09"):
    _r2 = subprocess.run(
        [sys.executable, "-c",
         f"import sys; sys.path.insert(0, {_B!r}); import {ad}"],
        capture_output=True, text=True, cwd=_B)
    ok(_r2.returncode == 0, f"{ad} TEK BASINA import (yalniz model_09/)",
...
# `taban_09.veri_kur` icinde assert'le kapali; burasi BAGIMSIZ olarak
...
# `IZ_09` ELLE YAZILI bir sabit ve kimse onu denetlemiyordu. Defter
# eski izle kaldi -- 7. hucredeki `assert _iz == IZ_09` KOSUYU
...
if not os.path.exists(_nbY):
    ...
    _m = _re.search(r'IZ_09\s*=\s*"([0-9a-f]+)"', _src)
    ok(_m is not None, "defterde IZ_09 tanimli")
    if _m:
        ok(_m.group(1) == _iz_ger,
           "defterdeki IZ_09 = GERCEK olcme izi",
    ...
    # diye onlarca kilit tasiyor ve bunlar `ayar_09.py`nin KOPYASI --
    ...
    ok(not _kot, f"defterin {len(_kilit)} AYAR kilidi ayar_09 ile TUTUYOR",
       str(_kot))
    ok(len(_kilit) >= 20,
    ...
    # SESSIZ kalamaz.
    _ARAC = {"PENCERE": "pencere_09.py", "ASAMA1": "asama1_09.py",
             "TANI": "tani_09.py", "ANALIZ": "analiz_09.py",
             "NULL": "null_09.py", "KONUS": "konus_09.py"}
    _kotu = []
...
# bolmesi degil.
print("\n=== 7d) NULL TABANI ===")
import null_09 as _NL                                       # noqa: E402
...
# degerle karsilastiriliyor. `veri_09` degisirse assert DUSER -- yani
...
print("\n" + "=== 8) KONUS_09 BEYANI ===")
import io                                                    # noqa: E402
import konus_09 as _K                                        # noqa: E402
...
# (`kos_09.py` MODEL="model_08", `ayar_09.py` from taban_08).
#
...
_ks = io.open(os.path.join(_B, "konus_09.py"), encoding="utf-8").read()
