# -*- coding: utf-8 -*-
"""model_09 — KLASIK decoder-only transformer. Bu projenin HICBIR ozelligi YOK.
...
NE VAR -- 2026'nin standart kucuk-LLM tarifi
    ...
    SwiGLU, d_ff = 8/3 * d            GELU + 4d YERINE (%3 FAZLA param)
...
`test_09.py` her kosuda sinar: VERI alanlari model_b15 ile birebir
...
PARAMETRE
    ...
    SwiGLU   3 d d_ff         = 540.672      (d_ff 704 = 8/3*256, 64'e yuvarli)
             GELU+4d karsiligi  524.288      -> SwiGLU %3,1 FAZLA,
                                               8 katmanda +131.072.
             'ayni parametre' YAZIYORDU, YANLIS (dis hakemlik, 18 Eylul).
             8/3 orani LLaMA'nin; 704'e yuvarlama BIZIM -- LLaMA 256'nin
             katina yuvarlar (d=256 icin 768) ve bunu tensor core icin
             degil TENSOR PARALLELISM bolunebilirligi icin yapar. Biz
             sharding yapmiyoruz, 64 dogru tanecik.
...
# EKLENMEZ -- kullanici karari, 16 Eylul: "model_09 diger hicbir model
...
assert hasattr(M, "egit"), (
    f"taban_09 MODUL degil PAKET olarak yuklendi: {getattr(M,'__file__',None)}")
...
def _rope(x, cos, sin):
    ...
    !! DONME fp16'DA YAPILIYOR (dis hakemlik, 18 Eylul: "LLaMA/HF
    rotasyonu fp32'de yapip sonra cast eder"). DOGRU, ve BEDELI OLCULDU
    (t_len 512, sabit iki vektor, m-n=3):

        fp32   skor -8.281103   GORELILIK YAYILIMI 4,01e-05
        fp16   skor -8.285731   GORELILIK YAYILIMI 3,93e-03
        fark   2,08e-03  ->  skorun %0,025'i

    Yayilim fp16'da 100 KAT kotu, ama mutlak buyukluk skorun binde
    birinin dorde biri. KABUL EDILDI cunku t_len 512 -- uzun baglamda
    (32k+) bu karar YENIDEN OLCULMELIDIR.
...
class ModelSade(nn.Module):
    ...
    def __init__(self, ayar: M.Ayar, vocab: int):
        super().__init__()
        assert ayar.dongu == 1, (
            f"model_09 DONGUSUZ: dongu={ayar.dongu}. Dongu istiyorsan "
            f"model_b ailesini kullan.")
        assert ayar.dar_alfa == 0 and not ayar.dar_kapi, (
            "model_09'da Phi DARBOGAZI YOK -- dar_alfa=0, dar_kapi=False")
        assert ayar.kopru_kayip == 0, "model_09 YARDIMCI KAYIP KULLANMAZ"
        assert ayar.mask_poz is None and not ayar.mask_blok, (
            "model_09'da MASKE YOK")
        self.ayar = ayar
    ...
    def n_param(self, bagli_ayri=False):
        ...
        `parameters()` zaten `remove_duplicate=True` -- eski surumdeki
        `{id(p): p}` sozlugu GEREKSIZDI (olculdu: iki yol da 6.443.008).
        `bagli_ayri=True` gomme iki kez sayilan degeri dondurur; baska
        depolarin 'parametre sayisi' rakamiyla kiyaslarken gerekiyor."""
    ...
    def govde(self, x):
        ...
        `asama1_09.gizli()` (DOGRUSAL SONDA) bunu cagirir. KUSUR ve
...
# `ayar_09.py`de tek tek yazili -- ve artik `model_b15`ten DEVRALINMIYOR,
# `Ayar()` varsayilaninin uzerine acikca yaziliyor.
from ayar_09 import AYAR, GOREV_ALAN                         # noqa: E402,F401
