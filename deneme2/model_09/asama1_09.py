# -*- coding: utf-8 -*-
"""asama1_09 — DOGRUSAL SONDA: kopru gizli durumda VAR MI?

    python asama1_09.py <kosu klasoru> [--adim N] [--bolme comp]
...
def gizli(net, sv: OLC.Sinav, bs=256):
    ...
    `net.emb + net.pos` yaziliydi ve `ModelSade`de `.pos` YOK (RoPE);
...
def main():
    ...
    print(f"=== asama1_09  {ayar.ad} t{ayar.tohum}  adim {adim} ===")

    v = M.veri_kur(ayar, yaz=SUS)
    G = importlib.import_module(ayar.veri_ad).kur(ayar.veri_tohum)
    _X, S = KOR.havuz(v, G, ayar.kopya, ayar.t_len, ayar.veri_tohum,
                      ayar.tetik, yaz=SUS)
