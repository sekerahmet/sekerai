# -*- coding: utf-8 -*-
"""ayar_09 — BU KOLUN KENDI ayari. TEK BASINA DURUR.
...
IKI GRUP, IKI GEREKCE
  ...
  GOREV alanlari    -> model_b15'in gordugu sinavla AYNI KALMASI
                       ...
                       ve ikisi de BU KOLUN TANIMI:
                         ...
                         ek_kip   "tr" -> "tr2"
                       `test_09.py` bunlari BILDIRILMIS AYRISMA diye
...
from taban_09 import Ayar                                    # noqa: E402

# Sinavin AYNI kalmasi GEREKEN alanlari. `test_09.py` bunlari model_b15
...
# Dolayisiyla `test_09` artik "veri_09 == veri_okul4" demiyor;
# `veri_09`in KENDI IDDIALARINI siniyor (sema, soy agaci, zincir
...
AYAR = Ayar(
    ...
    veri_ad="veri_09",   # KENDI veri modulu -- ve BU KOLUN DUGMESI.
    ...
    #
    #   bicim=3       -> metin_09.BICIM, DORT yuzey, cumle basina RASTGELE
    ...
    #   ident_frac    -> kimlik BELGESI (korpus_09.kimlik_belgeleri)
    #
    ...
    #
    # CLAUDE.md kural 1: 20.000 ILK SINIR, tavan DEGIL. 60.000 BASTAN
    # yaziliyor cunku esit maruziyetin cozumu bu; gerekce OLCULENLER
    # §2'ye gider.
    batch=32,
    adim=60000,          # ILK SINIR 20.000 DEGIL -- esit maruziyetin cozumu
    #                      bu (CLAUDE.md kural 1: tavan YOK, gerekce yazilir).
    ...
    # zemini oydu.
    isinma=6000,
    olc_her=6000,        # 10 olcum noktasi (model_08 ile AYNI sayi)
    ort_bas=18000,       # kosunun %30'u -- model_08'de 6000/20000

    ort_her=6000,        # olc_her ile AYNI -- her olcum noktasi bir esik.
...
assert AYAR.adim == 60000, (
    "adim MARUZIYETTEN turetildi: 32*512*60000 / 22.320.526 = 44,0 gecis "
    "x 10 cumle/olgu = 440 maruziyet (model_08: 442). Uc sayidan biri "
    "degisirse digerleri de degismeli.")
assert abs(AYAR.batch * AYAR.t_len * AYAR.adim / 22_320_526 * 10 - 440) < 6, (
    "MARUZIYET DENKLEMI: batch * t_len * adim / korpus * 10 ~ 440")
...
assert AYAR.veri_ad == "veri_09", "BU KOL KENDI veri modulunu okur"
