# -*- coding: utf-8 -*-
"""pencere_09 — BU KOLUN KENDI BIRINCIL OKUMASI. TEK BASINA DURUR.
...
GECMEZ; `test_09.py` ikisinin AYNI SEYI olctugunu her kosuda siniyor.
"""
...
import korpus_09 as KOR                                       # noqa: E402
...
def ayar_oku(klasor: str) -> M.Ayar:
    ...
    # Tablo `taban_09.ESKI_VARSAYILAN` -- TEK yerde durur, `surdurme_oku`
...
@torch.no_grad()
def olc(ayar: M.Ayar, veri: M.Veri, S, sinav: dict, sd: dict) -> dict:
    """Bir agirlik kumesini BUTUN bolmelerde olcer.

    !! model_08'in UC IKINCIL olcusu DUSTU:
      `ek_*`        bildirme ekini AYRI olcuyordu, cunku orada `dir` bir
                    JETONDU ve birincil olcu onu maskeliyordu. Karakter
                    duzeyinde ek zaten cevabin parcasi -- TAM ESLESME
                    onu hesaba katiyor, ayri sutuna gerek yok. Ayrimi
                    `yakin` sutunu tutuyor: varlik dogru, ek yanlis.
      `soru_*`      sinav ZATEN soru yuzeyinde. Ayri yuzey kalmadi.
      `fim_*`       FIM cikti (kullanici karari, 18 Eylul).
    """
...
def main():
    ...
    # !! KORPUS BURADA DA KURULUYOR. Sozluk ondan cikiyor ve modelin
    # gomme katmani ona gore boyutlandi; ezbere 63 yazmak, korpus
    # degisince SESSIZCE yanlis sozluk kurardi.
    _X, S = KOR.havuz(veri, importlib.import_module(ayar.veri_ad)
                      .kur(ayar.veri_tohum), ayar.kopya, ayar.t_len,
                      ayar.veri_tohum, ayar.tetik,
                      yaz=lambda *x, **y: None)
    G = importlib.import_module(ayar.veri_ad).kur(ayar.veri_tohum)
    sinav = {k: OLC.Sinav(S, veri, G, L[k], 1 if k == "one" else 2, k)
             for k in L if len(L[k])}
    # OLCME SETI PARMAK IZI -- taban_09.olcme_izi()'nden, KOPYA DEGIL.
    iz = M.olcme_izi(L)
    ...
    _ay = sorted(glob.glob(os.path.join(a.klasor, "ayar_t*.json")))
    if not _ay:
        raise SystemExit(
            f"\n!! {a.klasor} icinde ayar_t*.json YOK.\n"
            "   Bu klasor bir kosunun ciktisi degil, ya da kosu ayar\n"
            "   dosyasini yazmadan kesildi. Olcme izi KARSILASTIRILAMAZ,\n"
            "   yani bu sayilarin EGITILEN orneklere ait oldugu\n"
            "   DOGRULANAMAZ -- olcum YAPILMIYOR.")
    # `sorted`: birden fazla tohum varsa glob SIRASI isletim sistemine
    # bagliydi. Izler ayni oldugu icin sonuc degismiyor, ama hangi
    # dosyanin okundugu TEKRARLANABILIR olsun.
