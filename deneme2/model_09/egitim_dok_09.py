# -*- coding: utf-8 -*-
"""egitim_dok_09 — VERININ KENDISI, OKUNAN metin.
...
SINAV bolmeleri havuzda YOK (tutuluyorlar) -- onlar `pencere_09`nin
cagirdigi AYNI kodlayicilarla kuruluyor.

    python egitim_dok_09.py [--klasor <yol>] [--limit N]
"""
...
# duruyor -- ama ANAHTARLARI `veri_09.ILISKI` ile birebir tutmak ZORUNDA:
...
# OLCU AILESI -> 11'deki BOLUM BASLIGI. `pencere_09`nin urettigi her
...
def kur_03_iliskiler(d, v, H):
    ...
    assert not eksik, (
        f"ILISKI_NOT ile veri_09.ILISKI TUTMUYOR: {sorted(eksik)} "
...
def _duz_giris(d, v, H, ayar, hop):
    ...
    g += [
        ...
        "cakisma 0). `pencere_09`nin `ek_` sutunu onu AYRICA olcer.",
        "",
    ...
    if getattr(ayar, "soru_kat", 0) > 0:
        for bad in ("one", "seen", "comp", "ent", "ent_yok", "ood"):
            ...
            ekle(f"SORU BICIMI  ·  {bad}",
                 [SINAV_NOT[bad][1],
                  "AYNI zincir, soru yuzeyi. `pencere_09` -> soru_" + bad],
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=LIMIT,
                    help=f"her TIPten en fazla N satir (varsayilan {LIMIT}, "
                         "0 = HEPSI)")
    a = ap.parse_args()
    kl = a.klasor or os.path.join(_K, "veri", "model_09")
    ...
    with io.open(yol, "w", encoding="utf-8", newline="") as f:
        f.write("model_09 — VERI DOKUMU" + NL)
        f.write("=" * 72 + NL + NL)
        ...
        f.write("sirali, ilk N satir hep ayni birkac varligi gosterirdi."
                + NL)
        f.write("Tamamini dokmek icin:  python egitim_dok_09.py --limit 0"
                + NL + NL)
