# -*- coding: utf-8 -*-
"""taban_09 — BU KOLUN KENDI motoru. TEK BASINA DURUR.
...
Bunun icin `test_09.py` her kosuda DAVRANIS ESDEGERLIGI siniyor:
...
# `veri_09.py`, ve o BU klasorde. Veri modulu `veri_kur` icinde
# `ayar.veri_ad`dan import ediliyor (asagida), burada degil.

import jeton_09 as J
import korpus_09 as KOR
import olcme_09 as OLC
...
class Ayar:
    ...
    veri_ad: str = "veri_09"     # HANGI GRAF. "veri_okul2" = tam IKI KATI.
...
# ayni hata iki kez oldu (kos_09.py MODEL="model_08", ayar_09.py
...
def egitim_havuzu(ayar: Ayar, v: Veri, yaz=print):
    ...
    X, S = KOR.havuz(v, V09.kur(ayar.veri_tohum), ayar.kopya, ayar.t_len,
                     ayar.veri_tohum, ayar.tetik, yaz)
    return X, S
...
#
# Yerine `olcme_09.Sinav` + `olcme_09.olc`: model soruyu alir, gerisini
...
def surdurme_oku(yol, ayar: Ayar, model, opt, scaler, rs, iz, yaz=print,
    ...
    # ikisi de EGITIM METNINI kollamiyor. `metin_09.cumle`de tek bir ek
...
def erken_teshis(r: dict, ayar: Ayar, yaz=print, uyarildi: set | None = None):
    ...
    if k is not None and k == k and abs(k) > 1e-9:      # k == k  ->  NaN degil
        raise SystemExit(
            ...
            + os.linesep
            + "   isaret edecegi varlik GRAFTA YOK. `olcme_09.sorular` o"
            + os.linesep
...
# kosulabiliyordu. `taban_09` bir MOTOR; model_09'in ayari `ayar_09.py`de,
# kosuyu baslatan `model_09.py`. Burada calistirilacak bir sey YOK.
