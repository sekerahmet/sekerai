# -*- coding: utf-8 -*-
"""taban_01 — model_01'in KENDI motoru. TEK BASINA DURUR.

Kullanici karari, 16 Eylul 2026:
    *"bunlarin hepsi model_01 folderi altinda olmali. model_01 diger
    hicbir model ile ayni seyi kullanmamali."*

Bu dosya `model_a.py`nin KOPYASIDIR ve artik model_01'a aittir.
...
KENDILIGINDEN gelmez; model_01 ile model_b15 O GUNDEN SONRA FARKLI
KODLA olculmus olur. Bu, kopyanin gercek riski ve gozden kacarsa
sayilari sessizce karsilastirilamaz hale getirir.

Bunun icin `test_01.py` her kosuda DAVRANIS ESDEGERLIGI siniyor:

    egitim havuzu       taban_01 vs model_a  -> BIT DUZEYINDE ayni
...
# import ediyordu. `taban_01` bunu YAPMIYOR: model_01'in verisi
# `veri_01.py`, ve o BU klasorde. Veri modulu `veri_kur` icinde
...
class Ayar:
    ...
    veri_ad: str = "veri_01"     # HANGI GRAF. "veri_okul2" = tam IKI KATI.
    ...
    # --- BU KOLUN DUGMESI: egitim havuzundaki 2-hop ORANI ---------------
    #   1.0  hepsi  -> taban_01 ile taban_01 BIT DUZEYINDE ayni havuz
    #   0.0  HICBIRI -> model EGITIMDE tek bir 2-hop ornegi GORMEZ
    #   arasi: tr2'den DETERMINIST alt orneklem (tohum + 4242)
    #
    # NEDEN. Olculdu (16-17 Eylul): parcalar tek tek sorulunca 1.0000,
    # birlesince 0.0550-0.2550. Kopru ne dogrusal sondada, ne hizalanma
    # olcumunde, ne de cevabin kendisinde GORUNUYOR. Ama egitimde 68.436
    # adet 2-hop ornegi VAR -- yani EZBER, egitim hedefinin TAM bir
    # cozumu ve bilesim devresine hic basinc dogmuyor.
    #
    # Literaturdeki iki referans (2505.17923, 2509.24653) egitimde 2-hop
...
ESKI_VARSAYILAN = {
    ...
    "tam_kayip": False,
    "hop2_pay": 1.0,          # 17 Eylul'de EKLENDI; eski kosularda hepsi vardi
...
def egitim_havuzu(ayar: Ayar, v: Veri, yaz=print):
    ...
    # --- EGITIME GIRECEK 2-HOP SATIRLARI (ayar.hop2_pay) ----------------
    assert 0.0 <= ayar.hop2_pay <= 1.0, f"hop2_pay: {ayar.hop2_pay}"
    if ayar.hop2_pay >= 1.0:
        _tr2 = v.tr2                      # DEGISMEDI: bit duzeyinde ayni
    elif ayar.hop2_pay <= 0.0:
        _tr2 = []
    else:
        _r = np.random.RandomState(ayar.tohum + 4242)
        _ix = _r.permutation(len(v.tr2))[:int(round(ayar.hop2_pay * len(v.tr2)))]
        _tr2 = [v.tr2[i] for i in sorted(_ix)]
    if len(_tr2) != len(v.tr2):
        yaz(f"  !! 2-HOP KISILDI: {len(v.tr2)} -> {len(_tr2)} "
            f"(hop2_pay={ayar.hop2_pay})")
        if not _tr2:
            yaz("     EGITIMDE TEK BIR 2-HOP ORNEGI YOK. `seen` bolmesi de")
            yaz("     artik EGITILMEMIS zincirleri olcuyor -- SAGLIK-EZBER")
            yaz("     kapisi bu kolda TANIMSIZ (gevsetilmedi, ANLAMSIZ).")
    for _b in range(_bic):
        parca.append(kodla_1hop(v, v.one, _b))
        _kt.append(np.full((len(v.one), _nk), -1, np.int64))
        if _tr2:
            parca.append(kodla_2hop(v, _tr2, _b))
            _kt.append(np.array([_e(v, x[3])[:_nk] for x in _tr2], np.int64))
    _n_tab = sum(len(a) for a, _, _ in parca)
    if _bic > 1:
        yaz(f"  BICIM CESITLILIGI: {_bic} yuzey bicimi -- OLGU ve SORU "
            f"satirlarinin IKISINDE de")
        yaz(f"     (1hop {len(v.one)} + 2hop {len(_tr2)}) x{_bic} = {_n_tab}")
...
# kosulabiliyordu. `taban_01` bir MOTOR; model_01'in ayari `ayar_01.py`de,
# kosuyu baslatan `model_01.py`. Burada calistirilacak bir sey YOK.
