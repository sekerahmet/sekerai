# -*- coding: utf-8 -*-
"""model_01 — KLASIK decoder-only transformer. Bu projenin HICBIR ozelligi YOK.
...
`test_01.py` her kosuda sinar: VERI alanlari model_b15 ile birebir
...
# EKLENMEZ -- kullanici karari, 16 Eylul: "model_01 diger hicbir model
...
assert hasattr(M, "egit"), (
    f"taban_01 MODUL degil PAKET olarak yuklendi: {getattr(M,'__file__',None)}")
...
class ModelSade(nn.Module):
    ...
    def __init__(self, ayar: M.Ayar, vocab: int):
        super().__init__()
        assert ayar.dongu == 1, (
            f"model_01 DONGUSUZ: dongu={ayar.dongu}. Dongu istiyorsan "
            f"model_b ailesini kullan.")
        assert ayar.dar_alfa == 0 and not ayar.dar_kapi, (
            "model_01'da Phi DARBOGAZI YOK -- dar_alfa=0, dar_kapi=False")
        assert ayar.kopru_kayip == 0, "model_01 YARDIMCI KAYIP KULLANMAZ"
        assert ayar.mask_poz is None and not ayar.mask_blok, (
            "model_01'da MASKE YOK")
        self.ayar = ayar
    ...
    def govde(self, x):
        ...
        `asama1_01.gizli()` (DOGRUSAL SONDA) bunu cagirir. KUSUR ve
...
# `ayar_01.py`de tek tek yazili -- ve artik `model_b15`ten DEVRALINMIYOR,
# `Ayar()` varsayilaninin uzerine acikca yaziliyor.
from ayar_01 import AYAR, GOREV_ALAN                         # noqa: E402,F401
