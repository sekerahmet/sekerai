# -*- coding: utf-8 -*-
"""birim_17 -- KORPUS METNI -> BIRIM AKISI -> PENCERELER.
...
model_17'e OZGU iki sey var, ikisi de DENKLEM.md'den:
...
(birim yolunda KARAKTER YOK -- jeton_17 yalniz karakter korpusunda)

  veri_17     graf: 1608 varlik, 24 iliski, olgular
  metin_17    graf -> duz Turkce cumle
  korpus_17   cumle -> BELGE          (sayfalar / kimlik / reddetme)
  birim_17    belge -> satir satir BIRIM AKISI + <belge> siniri   <-- BU DOSYA
...
def belgeler(ayar, v=None, yaz=print):
    ...
    `korpus_17.havuz()` dort grubu (bildirim / soru / kimlik / reddetme)
...
def iz(b: Birim) -> str:
    """Birim dunyasinin parmak izi -- model_17'un GERCEKTEN egitildigi sey.

    model_11/13'un `korpus_izi` KARAKTER tensorunun md5'iydi. model_17
...
def kaydet(b: Birim, yol: str, yaz=print) -> str:
    ...
    164 MB onbellek) Colab'da hic kosmaz -- model_17 karakter gormuyor."""
...
def kur(ayar, v=None, yaz=print, onbellek: str | None = None) -> Birim:
    ...
    yazilir. Colab bu yolu Drive'daki `model_17/` klasorune verir ve
    korpus ORADA URETILMEZ.

    Bolme OGRENILMIYOR, BILINIYOR: Turkce ekler kapali bir kume
    (`ek_17`). BPE denendi ve `Bahcelievler`i `B|ah|c|eli|ev|l|er`
    ...
    # OZEL AD yuzeyleri -- HAM, cevrilmemis. `ek_17` bunlari ham
    ...
    assert not eksik, (
        "%d kelime BILINEN KOK + BILINEN EK'e cozulemiyor -- `ek_17.KOK` "
