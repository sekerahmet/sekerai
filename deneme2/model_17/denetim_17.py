"""denetim_17 -- veri yolunun kapilari.  Her kapinin NEDEN var oldugu yazili.
...
model_17'yi ilgilendirmiyor.  Buraya VERI YOLUNA ait olanlar tasindi;
her birinin gerekcesi bir KAYIPTAN geliyor, tahminden degil.

    python denetim_17.py
...
(model BIRIM goruyor -- karakter yalnizca ara adim ve kapi)
  ...
  dok_17      goz ile okunur dokum
  denetim_17  KAPILAR   <-- BU DOSYA
...
def _veri():
    ...
    if _D is None:
        ...
        yol = os.environ.get("KORPUS_PT",
                             r"G:/Drive'ım/model_17/korpus_17.pt")
...
def _0():
    """Kol KENDI kodunu tasir.  model_17.py baska bir kol dosyasina
    baglanirsa kol tek basina kosturulamaz hale gelir."""
    src = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "model_17.py"), encoding="utf-8").read()
...
def _1():
    """*Gerekce (birim_17 yorumu):* onceki hal son pencereyi DUSURUYORDU.
    `sliding_window_view` len-L+1 pencere verir ve sonuncusu gecerlidir."""
    import numpy as np
    import birim_17 as B
...
def _2():
    """*Gerekce (ek_17):* soyma ancak kalan BILINEN bir koke inerse kabul.
...
def _3():
    ...
    ve `olcme_17.yuzeyler` ikisini de OKUMALI."""
    import olcme_17 as O
    d = _veri()
    top = 0
...
def _5():
    ...
    import olcme_17 as O
    yol = os.environ.get("BIRIM_NPZ", "birim_17.npz")
    if not os.path.exists(yol):
        return f"ATLANDI -- birim dunyasi yok ({yol})"
    import birim_17 as BR
...
if __name__ == "__main__":
    import denetim_17  # noqa: F401  (kapilar import aninda kosar)
    print("=" * 74)
