...
class Ayar:
    ...
    veri_ad: str = "veri_08"     # HANGI GRAF. "veri_okul2" = tam IKI KATI.
...
def _dir(v, a=None, r=None):
    """Bildirme eki jetonu, ONCEKI KELIMEYE gore.
        Yilmaz -> 'dir   Celik -> 'tir   annesi -> dir   bolumu -> dur

    `r` verilirse ILISKIYE takilir (model_06: "annesidir"). Tablo veri
    modulunun; motor unlu uyumu HESAPLAMAZ."""
    if r is not None:
        return v.dir0 + int(v.dir_rel[int(r)])
...
def _soru(v, e):
    ...
    Cevabin tipi zaten ILISKIDEN belirli (olculdu: 24/24 iliskide hedef
...
def _bildirim(v, oz, il, az, a, son_r, bicim_no):
    """BILDIRIM cumlesi, UC TURKCE SIRADA. Doner: (dizi, cevabin ilk konumu).

    Kullanici karari, 17 Eylul: *"yalniz bildirim kullanalim"* ve
    *"Annesidir Fatma Yilmaz Ayse Yilmaz'in, bu da olur"*.

        0 KANONIK       Ibrahim Yilmaz'in kardesi Ozlem Yilmaz'dir.
        1 YUKLEM BASTA  Ozlem Yilmaz'dir Ibrahim Yilmaz'in kardesi.
        2 ILISKI BASTA  Kardesidir Ozlem Yilmaz Ibrahim Yilmaz'in.

    NEDEN UC SIRA: Turkce'de oge sirasi serbest, grameri EK tasiyor
    ('in sahip, -dir yuklem). Tek sirada egitilen model rolu KONUMDAN
    okur; sira degisince ekten okumak zorunda kalir. Olculdu (model_05):
    bir varliktan sonra gelen jeton cesidi 2 idi ve `ent` 0.0423'te
    kaldi.

    NEDEN SORU BICIMI YOK: olculdu (model_05) -- soru bicimi diziyi
    21,2 jetona cikariyordu ve cevap cumlesinin 7 jetonu sorunun
    BIREBIR KOPYASIYDI; o konumlarin kosullu entropisi 0,000, yani
    gradyan uretmiyorlardi. t_len 24 -> 14 duser. "Soru sorma" isini
    artik BOSLUK DOLDURMA yapiyor.

    !! 1 ve 2'de cevap cumlenin BASINDA, yani soldan tahmin EDILEMEZ.
    Bu bir kusur degil, o bicimlerin isi: orada model cevaptan OZNEYI
    cikarmayi ogreniyor (ters yon). Olcum HEP bicim 0 ile yapilir.
    """
    K = _kesme(v)
    D = _dir(v, a=a)
    ...
    # Ikisinde de cevap soldan TAHMIN EDILEMEZ (1608 varliktan hangisi
...
def kodla_1hop(v: Veri, batch, bicim_no: int = 0):
    """1-HOP satiri -- BILDIRIM, uc Turkce sirada. Ayrinti: `_bildirim`."""
    X = _bos(len(batch), v)
    P, T = [], []
...
def kodla_2hop(v: Veri, batch, bicim_no: int = 0):
    """2-HOP satiri -- BILDIRIM, uc Turkce sirada. Ayrinti: `_bildirim`.

        0 KANONIK       Ibrahim Yilmaz'in arkadasinin memleketi Kocaeli'dir.
        1 YUKLEM BASTA  Kocaeli'dir Ibrahim Yilmaz'in arkadasinin memleketi.
        2 ILISKI BASTA  Arkadasinin memleketidir Kocaeli Ibrahim Yilmaz'in.

    KOPRU DIZIDE YOK -- ne oznede ne cevapta. Model onu YAZMADAN
...
def kodla_fim_sinav(v: Veri, batch, hangi: str):
    ...
    Tek jetonlu adlarda (1608'in 81'i -- iller) bosluk ADIN TAMAMI olur; bu
    kacinilmaz ve zaten 1 jetondur.

    !! NEDEN AYRI FONKSIYON: bu dizi kurulumu `pencere_08.fim_dogruluk`in
...
def kodla_soru(v: Veri, batch, hop: int):
    """SORU + KISA CEVAP. `model_07`nin eklentisi.
...
def kodla_fim(v: Veri, dz, poz, uzunluk: int = 1):
    """BILDIRIM dizisini BOSLUK DOLDURMA dizisine cevirir.
        ...
        girdi   Fatma Yilmaz ' in annesi Ayse Yilmaz ' dir .
        poz=1   Fatma <BOS> ' in annesi Ayse Yilmaz ' dir . <AYIR> Yilmaz
                                                                   ^^^^^^ hedef

    Model NEDENSEL: boslugun sagini o konumda goremez. O yuzden cikarilan
    parca SONA tasinir; boylece model cumlenin TAMAMINI gorup sonra
    parcayi yazar. Standart "fill-in-the-middle" duzeni.

    NEDEN: olculdu (model_05, 1-hop kanonik satir). Soldan bakinca
    soyadin tabani 1,866 nat, iliskininki 2,016 nat -- yani OGRENILEMEZ.
    Cumlenin tamami gorulunce ikisi de belirli hale geliyor. Bu kolun
    iddiasi tam olarak o 4,4 nat'i ogrenilebilir ise cevirmek.

    `uzunluk` > 1 ise BIR PARCA bosalir (ornegin adin tamami):
        Ibrahim Yilmaz'in ... -> <BOS> 'in ... <AYIR> Ibrahim Yilmaz
    Olcumde OZNE sorusu icin gerekli; egitimde varsayilan 1 (kullanici
    karari: "her konum esit olasilikla").

    Uzunluk: L - u + 1 (bosluk) + 1 (ayirac) + u = L + 2. `u`dan
    BAGIMSIZ -- yani t_len parca boyuyla buyumez.
    """
    assert 0 <= poz < len(dz) and uzunluk >= 1, (poz, uzunluk, len(dz))
    assert poz + uzunluk <= len(dz), (poz, uzunluk, len(dz))
    return (dz[:poz] + [v.bosluk] + dz[poz + uzunluk:]
            + [v.ayir] + dz[poz:poz + uzunluk])
...
def egitim_havuzu(ayar: Ayar, v: Veri, yaz=print):
    ...
    # --- BOSLUK DOLDURMA (FIM) ------------------------------------------
    # Kullanici karari, 17 Eylul: *"yeni cumleler degil. bizim
    # argumanlarimizi detayina kadar ogrenmeli. Fatma ..... 'in annesi
    # Ayse Yilmaz'dir gibi"*.
    #
    ...
    if getattr(ayar, "fim_kat", 0) > 0:
        assert getattr(v, "bosluk", 0), "fim_kat, ek_kip='tr2' ISTER"
        assert ayar.tam_kayip, (
            "fim_kat tam_kayip GEREKTIRIR: kayip yalniz cevap yuvalarinda "
            "hesaplanirsa bosluk hedefi (dizinin SONU) hic ogrenilmez.")
        _rs = np.random.default_rng(1000 + ayar.veri_tohum)
        _fx = []
        for _X, _, _ in list(parca):
            for _r in _X:
                _dz = [int(t) for t in _r if int(t) != PAD]
                for _p in _rs.choice(len(_dz), size=min(ayar.fim_kat, len(_dz)),
                                     replace=False):
                    _fx.append(kodla_fim(v, _dz, int(_p)))
        _FX = np.zeros((len(_fx), v.t_len), np.int64)
        for _i, _d in enumerate(_fx):
            assert len(_d) <= v.t_len, (len(_d), v.t_len)
            _FX[_i, :len(_d)] = _d
        # CEVAP YUVASI olcumune KATILMAZ: FIM satirinda "cevap" kavrami
        # yok, hedef dizinin sonunda ve tam kayip onu zaten ogretiyor.
        _FP = np.tile(np.arange(v.cevap_yuva), (len(_fx), 1))
        _FT = np.full((len(_fx), v.cevap_yuva), -1, np.int64)
        parca.append((_FX, _FP, _FT))
        _kt.append(np.full((len(_fx), _nk), -1, np.int64))
        yaz(f"  BOSLUK DOLDURMA: satir basina {ayar.fim_kat} varyant "
            f"-> {len(_fx):,} satir")
        yaz("     <BOS> girdide GORUNUR ama HEDEF DEGIL (kayipta PAD'e "
            "cevriliyor)")
    ...
    # --- IYELIK KISA YOLU (18 Eylul) ------------------------------------
    # SORU satirlariyla AYNI kodlayici (`kodla_soru`, hop=1): yuzey
    # 1-hop, cevap cok adimli. FIM'e GIRMIYOR, tipki soru satirlari gibi.
    _n_ksy = 0
    if getattr(ayar, "kisayol_kat", 0) > 0 and v.kisayol:
        _kl = list(v.kisayol)
        for _k in range(ayar.kisayol_kat):
            parca.append(kodla_soru(v, _kl, 1))
            _kt.append(np.full((len(_kl), _nk), -1, np.int64))
            _n_ksy += len(_kl)
    ...
    if _n_soru:
        ...
        yaz("     SINAV BUNU KULLANMIYOR -- olcum duz bildirimle; "
            "olcme_izi DEGISMEZ.")
    if _n_ksy:
        yaz(f"  IYELIK KISA YOLU: {len(KISAYOL_YOLU)} iliski "
            f"-> {_n_ksy:,} satir")
        if v.kisayol_atilan:
            _t = sum(n for _, n in v.kisayol_atilan)
            yaz(f"     {_t} satir URETILMEDI (tutulan veriye dokunuyordu): "
                + ", ".join(f"{k} {n}" for k, n in v.kisayol_atilan))
        yaz("     \"Ayse Sahin'in dekani kimdir?  Melek Aydin'dir.\"")
        yaz("     GRAFA KENAR EKLEMEZ -- olgu, zincir ve olcme_izi AYNI.")
    kimlik = None
            ...
            with torch.autocast(DEV, dtype=torch.float16,
                ...
                if ayar.tam_kayip:
                    ...
                    if getattr(v, "bosluk", 0):
                        # !! <BOS>'un YERINI tahmin etmek OGRENILEMEZ:
                        # hangi jetonun cikarildigi RASTGELE. Olculdu
                        # (model_06, FIM havuzu): <BOS> aday olan
                        # konumlarin entropisi 0,821 nat ve bu, kaybin
                        # %75'i olurdu -- duzeltmeye calistigimiz
                        # hastaligin daha buyugu. O hedef PAD'e cevrilip
                        # kayiptan DUSURULUR. Model boslugun NEREDE
                        # oldugunu GORUR (girdi olarak), ama nereye
                        # dusecegini TAHMIN ETMEZ.
                        _y = _y.clone()
                        _y[_y == v.bosluk] = PAD
                    kayip = F.cross_entropy(
                        lg_tam[:, :-1].float().reshape(-1, lg_tam.shape[-1]),
                        _y.reshape(-1), ignore_index=PAD)
                    ana_top += ana.detach()
                    ana_say += 1
                else:
                    kayip = ana
