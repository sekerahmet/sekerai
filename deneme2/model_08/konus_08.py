# -*- coding: utf-8 -*-
"""konus_08 — model_08 ile DUZ KONUSMA. Sablon YOK, denetim YOK.
...
verilir (onkayit `belge/onkayit/model_08.md`).
    ...
    python konus_08.py                      son 5'in AGIRLIK ORTALAMASI
    python konus_08.py --adim 8000          belli bir adim
    python konus_08.py --genislik 5         son 5'in agirlik ortalamasi
    python konus_08.py --klasor <yol>       baska bir kosu

model_08'DE UC TURLU KONUSABILIRSIN. `model_06`da yalniz ilk ikisi
...
KLASOR = os.path.join("G:" + os.sep, "Drive'ım", "model_08", "t0")
...
class Sozluk:
    ...
    def __init__(self, v, VM):
        ...
        assert v.paylasilan, "konus_08 PAYLASILAN sozluk bekler"
        self.kel = {}
...
def dene(D, S, n=300):
    """JETONLAYICI SINAMASI -- `python konus_08.py --dene`.
    ...
    Kilit testine (`test_08.py`) KONMADI: o test kosudan once calisan
...
def main():
    ...
    assert (D.v.vocab, D.v.n_ent, D.v.ek0) == (v.vocab, v.n_ent, v.ek0), (
        "depodaki model_08.AYAR ile KOSUNUN ayari ayristi -- sozluk tutmuyor")
    D.v = v                      # kosunun kendi verisi okunsun
    S = Sozluk(v, D.VM)

    print(f"model_08 t{ayar.tohum}   anlik goruntu "
