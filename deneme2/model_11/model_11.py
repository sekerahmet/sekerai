# -*- coding: utf-8 -*-
"""model_11 — KLASIK decoder-only transformer. Bu projenin HICBIR ozelligi YOK.
...
`test_11.py` her kosuda sinar: VERI alanlari model_b15 ile birebir
...
assert hasattr(M, "egit"), (
    f"taban_11 MODUL degil PAKET olarak yuklendi: {getattr(M,'__file__',None)}")
...
class ModelSade(nn.Module):
    ...
    def __init__(self, ayar: M.Ayar, vocab: int):
        super().__init__()
        assert ayar.dongu == 1, (
            f"model_11 DONGUSUZ: dongu={ayar.dongu}. Dongu istiyorsan "
    ...
    def govde(self, x):
        ...
        `asama1_11.gizli()` (DOGRUSAL SONDA) bunu cagirir. KUSUR ve
...
# `ayar_11.py`de tek tek yazili -- ve artik `model_b15`ten DEVRALINMIYOR,
# `Ayar()` varsayilaninin uzerine acikca yaziliyor.
from ayar_11 import AYAR, GOREV_ALAN                         # noqa: E402,F401
